...
let update_mp_table exp_tree ind = match exp_tree with
	| Binop (Impl, a, b) -> 
...
let rec find_axiom exp_tree axioms_tail ind = 
   	match axioms_tail with
	    | [] -> None 
	    | h :: t -> 
	    	checker_a_a := Hashtbl.create 1001;
	    	if (check_with_axiom h exp_tree) then 
	    		Some (ind)
	    	else 
	    		find_axiom exp_tree t (ind + 1)
...
let get_ass_from_h header = 
	...
	let ass_tree_list  = parse_ass_from_h_to_tree_list (header) in
